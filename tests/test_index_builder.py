...
def test_build_index_happy_path(
    mock_connect, mock_exists, mock_fetch_spy, mock_fetch_top
):
    ...
    mock_conn.execute.side_effect = [
        MagicMock(
            fetch_df=MagicMock(return_value=pd.DataFrame({"dummy": [1]}))
        ),  # stock_prices
        MagicMock(
            fetch_df=MagicMock(return_value=pd.DataFrame({"date": ["2024-01-01"]}))
        ),  # dates
        None,  # BEGIN
        None,  # CREATE TABLE
        None,  # INSERT
    ...
    mock_fetch_top.return_value = pd.DataFrame(
        {"ticker": [f"T{i}" for i in range(100)], "close": [1.0 for _ in range(100)]}
    )
...
def test_build_index_duckdb_missing(_, mock_log):
    ...
    mock_log.error.assert_called_once_with(
        f"DuckDB file not found: {Config.DUCKDB_FILE}"
    )
...
def test_build_index_no_valid_top_100(_, mock_connect, mock_fetch_top, mock_log):
    ...
    mock_conn.execute.side_effect = [
        MagicMock(
            fetch_df=MagicMock(
                return_value=pd.DataFrame({"ticker": ["A"], "close": [100]})
            )
        ),  # stock_prices
        MagicMock(
            fetch_df=MagicMock(return_value=pd.DataFrame({"date": ["2024-01-01"]}))
