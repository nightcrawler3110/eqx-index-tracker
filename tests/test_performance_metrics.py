...
def test_compute_metrics_success(mock_connect, mock_logger, mock_config, mock_path, mock_dataframes):
    ...
    mock_conn.execute.side_effect = lambda query: MagicMock(fetch_df=fetch_df_side_effect)
...
@patch('src.performance_metrics.logger')
@patch('src.performance_metrics.duckdb.connect')
def test_compute_metrics_empty_index_table(mock_connect, mock_logger, mock_config, mock_path):
    mock_config.DUCKDB_FILE = "mock.db"
    mock_path.return_value.exists.return_value = True
    ...
    mock_conn.execute.side_effect = lambda q: MagicMock(fetch_df=lambda: empty_df if "index_values" in q else spy_df)

    performance_metrics.compute_metrics()

    found = any("index_values table is empty." in str(call[0][0]) for call in mock_logger.error.call_args_list)
...
@patch('src.performance_metrics.logger')
@patch('src.performance_metrics.duckdb.connect')
def test_compute_metrics_empty_market_index(mock_connect, mock_logger, mock_config, mock_path):
    mock_config.DUCKDB_FILE = "mock.db"
    mock_path.return_value.exists.return_value = True
    ...
    empty_df = pd.DataFrame()
    mock_conn.execute.side_effect = lambda q: MagicMock(fetch_df=lambda: index_df if "index_values" in q else empty_df)

    performance_metrics.compute_metrics()

    found = any("market_index table is empty." in str(call[0][0]) for call in mock_logger.error.call_args_list)
...
@patch('src.performance_metrics.logger')
@patch('src.performance_metrics.duckdb.connect')
def test_compute_metrics_rollback_on_exception(mock_connect, mock_logger, mock_config, mock_path):
    mock_config.DUCKDB_FILE = "mock.db"
    mock_path.return_value.exists.return_value = True
    ...
    mock_conn.execute.side_effect = lambda q: MagicMock(fetch_df=lambda: pd.DataFrame({'wrong_column': [1]}))

    performance_metrics.compute_metrics()

    assert mock_conn.execute.call_args_list[0][0][0].startswith("SELECT")
    found = any("Error computing metrics" in str(call[0][0]) for call in mock_logger.error.call_args_list)
